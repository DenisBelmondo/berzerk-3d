...
void Renderer::initialize() {
    ...
    InitAudioDevice();
    playerBulletModel =
        LoadModelFromMesh(raylib::MeshUnmanaged::Cube(0.25F, -0.25F, 1.0F));
    wallMesh = GenMeshCube(1, 1, 1);
    UploadMesh(&wallMesh, false);
    wallShader =
        LoadShader("static/shaders/wall.vert", "static/shaders/wall.frag");
    wallShader.locs[SHADER_LOC_MATRIX_MVP] =
        GetShaderLocation(wallShader, "mvp");
    wallShader.locs[SHADER_LOC_VECTOR_VIEW] =
        GetShaderLocation(wallShader, "viewPos");
    ...
    sounds[std::size_t(SoundID::PISTOL_FIRE)] =
        LoadSound("static/audio/pistol_fire.wav");
    sounds[std::size_t(SoundID::ROBOT_EXPLODE)] =
        LoadSound("static/audio/robot_explode.wav");
    ...
    vSpriteCamera = raylib::Camera({}, { 0, 0, -1 }, { 0, 1, 0 }, 1.2F,
                                   CAMERA_ORTHOGRAPHIC);
}
...
void Renderer::render() {
    raylib::Vector3 playerPosition3d(world->playerTransform.position.x, 0,
                                     world->playerTransform.position.y);
    raylib::Vector3 playerDirection3d(world->playerTransform.direction.x, 0,
                                      world->playerTransform.direction.y);
    ...
    SetShaderValue(wallShader, wallShader.locs[SHADER_LOC_VECTOR_VIEW],
    ...
    vSpriteCamera.DrawBillboard(
        *vSpriteTexture,
        raylib::Vector3(0, -0.1F, -1) +
            raylib::Vector3(vSpriteOffset.x, vSpriteOffset.y, 0),
        1);
